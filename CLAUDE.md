...
**구현 구조**
- 모노레포 + **포트/어댑터(헥사고날)** 을 기본으로 잡는다. 상세는 `AGENTS.md` 「아키텍처」.
- 유스케이스·도메인 규칙을 CLI·DuckDB·fetch 스크립트에 직접 넣지 말고, 포트 뒤에 둔 뒤 어댑터로 연결한다.
- **TDD:** 도메인/유스케이스는 Vitest로 실패 테스트 먼저 (`pnpm test`). `AGENTS.md` 「테스트」.
